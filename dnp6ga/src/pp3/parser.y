...
%token   T_While T_For T_If T_Else T_Return T_Break T_Switch T_Default T_Case
...
%type <iDecl>     IntfDecl
%type <stmtList>  StmtList StmtList1 switchbody caselist 
%type <sb> StmtBlock
%type <stmt>    Stmt   
%type <cDecl>	  ClDecl
%type <expr> 	  ExprOpt  Constant Call Expr  LValue 
...
%left<op1>  '='
%left<op1> T_And T_Or
%left<op1> T_LessEqual T_GreaterEqual T_Equal T_NotEqual '>' '<'
%left<op1> '+' '-'
%left<op1> '*' '/' '%'
%left<op1>  '!' UMINUS T_Incr T_Decr
%left<op1> '.' '(' ')' '[' ']'
...
SwitchStmt: T_Switch '(' Expr ')' '{' switchbody '}'{$$ = new SwitchStmt($3,$6) ;}
          ;

switchbody: caselist optDefault{ ($$ = $1)->Append($2);};

optDefault: T_Default ':' StmtList1 {$$ = new DefaultStmt(NULL,$3);}
          |{}  ;
...
case: T_Case T_IntConstant ':' StmtList1 {$$ = new CaseStmt(new IntConstant(@2,$2),$4);}; 
...
Call: Expr '.' T_Identifier '(' Actuals ')' {$$ = new Call (@1,$1,new Identifier(@3,$3),$5);}
...
LValue: Expr '.' T_Identifier {$$ = new FieldAccess($1,new Identifier(@3,$3));}
       ...
       | Expr T_Incr %prec UMINUS {$$ = new PostfixExpr($1, new Operator(@2,"++"));}
     | Expr T_Decr  %prec UMINUS {$$ = new PostfixExpr($1,new Operator(@2,"--"));}
